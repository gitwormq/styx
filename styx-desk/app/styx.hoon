...
++  on-load
  ...
  =.  state
    ?-  -.old
        %1  old
        %0
      ::  migrate from state-0 to state-1
      ~&  >  '%styx migrating from state-0 to state-1'
      ::  migrate cargo: combine subject+body, add acked=~
      =/  new-cargos=(map @uv cargo)
        %-  ~(gas by *(map @uv cargo))
        %+  turn  ~(tap by cargos.old)
        |=  [id=@uv c=cargo-0]
        =/  new-content=cargo-content
          ?-  -.cargo-content.c
              %message
            =/  combined=@t  (crip "{(trip subject.cargo-content.c)}: {(trip body.cargo-content.c)}")
            [%message combined]
              %file
            [%file name.cargo-content.c mime.cargo-content.c data.cargo-content.c]
          ==
        [id [id recipient.c new-content created.c updated.c ~]]
      ::  migrate deliveries: combine subject+body, add cargo-id=0v0
      =/  new-deliveries=(list delivery)
        %+  turn  deliveries.old
        |=  d=delivery-0
        =/  new-content=delivery-content
          ?-  -.delivery-content.d
              %message
            =/  combined=@t  (crip "{(trip subject.delivery-content.d)}: {(trip body.delivery-content.d)}")
            [%message combined]
              %file
            [%file name.delivery-content.d mime.delivery-content.d data.delivery-content.d]
          ==
        [from.d 0v0 new-content sent.d is-test.d]
      :*  %1
          config.old
          crossing-status.old
          last-obol.old
...
++  on-poke
  ...
  ::
    ...
    ::  send ack back to sender (unless it's a test to self)
    ?:  &(is-test.del =(from.del our.bowl))
      `this
    :_  this
    :~  [%pass /ack/(scot %uv cargo-id.del) %agent [from.del %styx] %poke %styx-ack !>(`ack`[our.bowl cargo-id.del])]
    ==
  ::
      %styx-ack
    =/  a  !<(ack vase)
    ~&  >  "received ack from {<from.a>} for cargo {<cargo-id.a>}"
    ::  find cargo and mark as acked
    =/  carg  (~(get by cargos.state) cargo-id.a)
    ?~  carg  `this
    ::  verify ack is from actual recipient
    ?.  =(from.a recipient.u.carg)  `this
    =/  updated  u.carg(acked `now.bowl)
    =.  cargos.state  (~(put by cargos.state) cargo-id.a updated)
    :_  this
    [[%give %fact ~[/updates] %styx-update !>([%cargo-acked cargo-id.a now.bowl])]]~
  ::
      %handle-http-request
  ...
  ++  handle-action
    ...
    ::
      ...
      ?:  =(recipient.act our.bowl)
        ...
        `state
      =/  new  (make-cargo:lib now.bowl eny.bowl recipient.act body.act)
    ...
    ::
      ...
      ?~  old  `state
      =/  new  u.old(cargo-content [%message body.act], updated now.bowl)
    ...
    ::
      ...
      =/  test-del=delivery
        :*  from=our.bowl
            cargo-id=0v0  ::  dummy id for test
            delivery-content=[%message 'This is a roundtrip test from styx.']
            sent=now.bowl
            is-test=%.y
        ==
  ...
  ++  handle-http
    ...
    ?:  =(meth %'GET')
      ...
      ::  handle export endpoint separately (no obol registration)
      ?:  =(pax /styx/export)
        (serve-export id)
    ...
    ::
      ...
      =/  ship  (slaw %p u.rec)
      ?~  ship  (redirect id)
      =^  cards  state  (handle-action [%add-cargo u.ship u.bod])
      :_  this
      (weld (http-redirect id) cards)
    ::
        [%styx %delete ~]
  ...
  ++  render-export
    |=  [st=state-1 now=@da]
    ^-  octs
    =/  cargo-list=(list tape)
      %+  turn  ~(tap by cargos.st)
      |=  [id=@uv =cargo]
      (cargo-to-json:lib cargo)
    =/  cargo-json=tape
      ?~  cargo-list  "[]"
  ...
  ::
  ++  join-with
    |=  [items=(list tape) sep=tape]
    ^-  tape
    ?~  items  ""
    ?~  t.items  i.items
    (weld i.items (weld sep $(items t.items)))
  ::
  ++  redirect
    |=  id=@ta
    ^-  (quip card _this)
  ...
  ++  render-page
    ...
    =/  cargo-section=tape
      ...
      =/  content-raw=tape
        ?-  -.cargo-content.cargo
          ...
          %file     (weld "File: " (trip name.cargo-content.cargo))
        ==
      =/  content-escaped=tape  (html-escape content-raw)
      =/  is-long=?  (gth (lent content-raw) 250)
      =/  content-display=tape
        ?.  is-long  content-escaped
        ;:  weld
          "<span class='preview'>{(html-escape (scag 250 content-raw))}...</span>"
          "<span class='full' style='display:none'>{content-escaped}</span>"
          "<a class='toggle-link' onclick='toggleMsg(this)'>Show more</a>"
        ==
      =/  created-str=tape  (format-date:lib created.cargo)
      =/  ack-status=tape
        ?~  acked.cargo
          "<span style='color:#ff9f1c'>Pending</span>"
        "<span style='color:#4ecca3'>Delivered {(format-date:lib u.acked.cargo)}</span>"
      =/  delete-form=tape
        "<form method='post' action='/styx/delete' style='display:inline;float:right'><input type='hidden' name='id' value='{<id>}'><button style='background:#e94560;color:#fff;padding:0.25rem 0.5rem;font-size:0.8rem'>Delete</button></form>"
      ;:  weld
        "<div style='border:1px solid #8d99ae;padding:0.5rem;margin:0.5rem 0;border-radius:4px'>"
        delete-form
        "<div style='margin-bottom:0.5rem'>{content-display}</div>"
        "<div style='font-size:0.8rem;color:#8d99ae'>To: {<recipient.cargo>} | Created: {created-str} | {ack-status}</div>"
        "</div>"
      ==
    =/  cargo-display=tape
      ?:  =(~ cargo-section)
    ...
    =/  delivery-section=tape
      ...
      =/  content-raw=tape
        ?-  -.delivery-content.delivery
          ...
          %file     (weld "File: " (trip name.delivery-content.delivery))
        ==
      =/  content-escaped=tape  (html-escape content-raw)
      =/  is-long=?  (gth (lent content-raw) 250)
      =/  content-display=tape
        ...
        ;:  weld
          ...
          "<a class='toggle-link' onclick='toggleMsg(this)'>Show more</a>"
        ==
      =/  sent-str=tape  (format-date:lib sent.delivery)
      =/  test-tag=tape  ?:(is-test.delivery " <span style='color:#ff9f1c'>(TEST)</span>" "")
      ;:  weld
        "<div style='border:1px solid #4ecca3;padding:0.5rem;margin:0.5rem 0;border-radius:4px'>"
        "<div style='margin-bottom:0.5rem'>{content-display}</div>"
        "<div style='font-size:0.8rem;color:#8d99ae'>From: {<from.delivery>} | Received: {sent-str}{test-tag}</div>"
        "</div>"
      ==
    ...
    ;:  weld
      ...
      "<p style='color:#8d99ae;font-size:0.75rem;margin:0 0 0.5rem 0'>Recipient must have Styx installed to receive cargo.</p>"
