...
+$  cargo
  $:  id=@uv                    ::  unique identifier
      ...
      acked=(unit @da)          ::  when recipient acknowledged receipt
  ==
::
+$  cargo-content
  $%  [%message body=@t]
...
+$  config
  ...
  ==
::
::  +state-1: agent state (version 1 - current)
::
+$  state-1
  $:  %1
      =config
  ...
  ==
::
::  Legacy types for migration from state-0
::
+$  cargo-content-0
...
+$  delivery-0
  $:  from=ship
      delivery-content=delivery-content-0
      sent=@da
      is-test=?
  ==
::
+$  state-0
  $:  %0
      ...
      =crossing-status
      last-obol=(unit @da)
      crossing-time=(unit @da)
      cargos=(map @uv cargo-0)
      obols=(list obol)
      deliveries=(list delivery-0)
  ==
::
...
+$  action
  $%  ::  life signs
      ...
      ::  cargo management
      [%add-cargo recipient=ship body=@t]
      [%add-file recipient=ship name=@t mime=@t data=@]
      [%edit-cargo id=@uv body=@t]
      [%delete-cargo id=@uv]
      ::  configuration
...
+$  delivery
  $:  from=ship
      cargo-id=@uv              ::  original cargo id for ack
      =delivery-content
      sent=@da
      is-test=?
  ==
...
::
+$  ack
  $:  from=ship               ::  who is acknowledging
      cargo-id=@uv            ::  which cargo they received
  ==
::
